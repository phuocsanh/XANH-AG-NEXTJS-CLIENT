  ...
  useEffect(() => {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (window.scrollY <= 5 && e.touches[0]) {
        // Kiểm tra nếu đang chạm vào phần tử có scroll riêng (menu mobile, drawer, etc.)
        const target = e.target as HTMLElement;
        const isInsideScrollable = target.closest('[role="dialog"], .mobile-menu, [data-state="open"], .overflow-y-auto, .overflow-y-scroll');
